...
const result = sales.reduce((table, item) => {
  const category = item.category;

  // initialize for not exists
  if (!table[category]) {
    table[category] = {
      totalRevenue: 0,
      itemCount: 0,
  ...
  // revenue add
  table[category].totalRevenue += item.price * item.quantity;

  // count add
  table[category].itemCount += item.quantity;
